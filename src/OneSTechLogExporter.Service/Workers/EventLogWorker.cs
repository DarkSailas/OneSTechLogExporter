    ...
    ILogger<EventLogWorker> logger) : BackgroundService
{
    ...
    private async ValueTask ProcessEventLogsAsync(CancellationToken ct)
    {
        ...
        {
            logger.LogWarning("Словарь Журнала Регистрации (1Cv8.lgf) не найден в {RootDir}", rootDir);
            return;
        }

        logger.LogDebug("Парсинг словаря 1Cv8.lgf [{DictPath}]...", dictPath);
        var dictionary = await EventLogParser.ParseDictionaryAsync(dictPath, ct).ConfigureAwait(false);
